...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (zoneList.length === 0) {
      ...
      console.error('[cloudflare-analytics] zone lookup failed', { zoneId });
      // Do not echo the raw gql payload back to the client.
    ...
    if (!success) {
      // Errors might contain internal zone IDs / permission hints. Log full
      // detail server-side, return only the safe summary to the client.
      console.error('[cloudflare-analytics] fetch error', { errors });
      return res.status(500).json({
        success: false,
    ...
    return res.status(200).json({ success: true, dashboard: dashData });
  } catch (err) {
    console.error('[cloudflare-analytics] error:', err?.message);
    return res.status(500).json({ success: false, error: 'Cloudflare analytics request failed' });
  }
}
