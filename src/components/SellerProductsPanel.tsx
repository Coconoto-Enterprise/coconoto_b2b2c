...
import {
  Package, Plus, Pencil, Trash2, Loader2, AlertCircle,
  Tag, Box, ImageIcon, FileEdit, Save, X,
...
/**
 * Standalone seller products panel — used both by the legacy
 * `/seller-dashboard` page and embedded as a tab in the buyer dashboard
        ...
        <Card className="max-w-md text-center">
          <CardContent className="space-y-4 py-10">
            ...
            <p className="text-sm text-gray-600">
              Open the "Sell on Coconoto" tab to upgrade your account, then come back here to manage your products.
            </p>
          </CardContent>
        </Card>
      </div>
...
function ProductForm({
  vendorId, editingId, onCancel, onSaved, onError
}: {
  vendorId: string;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.product_name?.trim() || !form.description?.trim() || !(Number(form.price) > 0)) {
      ...
      onError('Product name, description and a valid price are required.');
      return;
    }
  ...
  return (
    <Card className="mb-2 border-emerald-200 shadow-sm">
      <CardHeader>
