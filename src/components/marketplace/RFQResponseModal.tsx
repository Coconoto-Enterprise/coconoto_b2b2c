...
interface RFQResponseModalProps {
  isOpen: boolean;
  onClose: () => void;
  rfq: {
    productName: string;
...
export function RFQResponseModal({ isOpen, onClose, rfq }: RFQResponseModalProps) {
  const { register, handleSubmit, reset, formState: { errors } } = useForm<ResponseFormData>();

  const onSubmit = (data: ResponseFormData) => {
    console.log('RFQ response submitted:', data);
    reset();
    onClose();
  };
            ...
            >
              <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                <div className="bg-blue-50 p-4 rounded-lg mb-6">
                  <h4 className="font-medium text-blue-900 mb-2">RFQ Details</h4>
                  <div className="grid grid-cols-2 gap-4 text-sm text-blue-800">
                    <div>
                      <p><strong>Product:</strong> {rfq.productName}</p>
                      <p><strong>Quantity:</strong> {rfq.quantity}</p>
                      <p><strong>Price:</strong> {rfq.price}</p>
                    </div>
                    <div>
                      <p><strong>Location:</strong> {rfq.location}</p>
                      <p><strong>Deadline:</strong> {new Date(rfq.expiryDate).toLocaleDateString()}</p>
                    </div>
                  </div>
                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Monthly Capacity</label>
                        <input
                          type="number"
